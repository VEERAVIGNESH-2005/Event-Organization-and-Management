...
const getEventImage = (title) => {
  ...
  if (lowerTitle.includes('food') || lowerTitle.includes('festival')) return foodfest;
  if (lowerTitle.includes('wedding') || lowerTitle.includes('ceremony')) return weddingImage;
  if (lowerTitle.includes('ear') || lowerTitle.includes('piercing')) return ear;
  if (lowerTitle.includes('house') || lowerTitle.includes('warming')) return house;
  if (lowerTitle.includes('corporate') || lowerTitle.includes('party')) return cor;
  if (lowerTitle.includes('music') || lowerTitle.includes('concert')) return music;
  if (lowerTitle.includes('diwali')) return diwali;
  if (lowerTitle.includes('navratri') || lowerTitle.includes('garba')) return nava;
  return weddingImage;
};
...
const EventsPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!discription.trim()) newErrors.discription = 'Description is required';
    if (!image.trim()) newErrors.image = 'Image URL is required';
    if (!amount.trim()) newErrors.amount = 'Budget is required';
    if (amount && isNaN(amount)) newErrors.amount = 'Budget must be a number';
      ...
      {eventdata.map((eacheve, index) => (
        <div className="event-division" key={index}>
          <div className="event-image">
            <img src={getEventImage(eacheve.title)} alt={eacheve.title} />
          </div>
      ...
      {formVisible && (
        <form className="event-form" onSubmit={handleSubmit}>
          ...
          <textarea
            ...
            required
            style={{borderColor: errors.discription ? 'red' : ''}}
            />
          {errors.discription && <span style={{color: 'red', fontSize: '12px'}}>{errors.discription}</span>}
          ...
          <button type="button" onClick={() => {setFormVisible(false); setErrors({}); setTitle(''); setdiscription(''); setimage(''); setamount('');}}>Cancel</button>
        </form>
      )}
    </div>
